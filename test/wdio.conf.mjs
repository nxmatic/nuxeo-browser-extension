...
import { nuxeo } from './features/step_definitions/support/client.mjs';

function extensionPath() {
  const dirname = path.dirname(fileURLToPath(import.meta.url));
  return path.join(dirname, '../dist/chrome');
}
...
export const config = {
  ...
  cucumberOpts: {
    require: ['./features/step_definitions/*.cjs', './features/step_definitions/*.mjs'], // <string[]> (file/dir) require files before executing features
  ...
  afterScenario(scenario) {
    ...
    return Promise.all(liveDocuments
      ...
      .then(() => nuxeo.repository().delete(userWorkspaces).catch((err) => {
        // No user workspaces
      }));
  },
  /**
