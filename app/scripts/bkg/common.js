...
const CONNECT_URL_STORAGE_KEY = 'com.nuxeo.connect.url';

// Use the new Promise-based functions
getFromStorage(CONNECT_URL_STORAGE_KEY)
  .then((entry) => {
    ...
    if (typeof entry === 'string') {
      ...
      return entry;
    } else if (typeof entry === 'object' && !Array.isArray(entry) && entry.hasOwnProperty(CONNECT_URL_STORAGE_KEY)) {
      // entry is a non-empty object (JSON map), retrieve the value
      return entry[CONNECT_URL_STORAGE_KEY];
    } else {
...
const setConnectUrl = window.setConnectUrl = (url) => 
  setToStorage({ 
    CONNECT_URL_STORAGE_KEY: url.toString(),
  }).then(() => {
